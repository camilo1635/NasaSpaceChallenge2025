...
export function NBLPlayerControls({ children }) {
  ...
  useFrame(() => {
    ...
    const speed = 0.08 // Velocidad más lenta para simular agua
    const forward = new THREE.Vector3(Math.sin(rotation.current.x), 0, Math.cos(rotation.current.x))
    const right = new THREE.Vector3(Math.cos(rotation.current.x), 0, -Math.sin(rotation.current.x))
    ...
    if (keys.current[' ']) dir.y += 1 // ESPACIO sube
    if (keys.current['shift']) dir.y -= 1 // SHIFT baja
    ...
    pos.y = Math.max(-8, Math.min(7, pos.y)) // Límite vertical
    pos.z = Math.max(-14, Math.min(14, pos.z))

    // Cámara detrás del jugador
    const cameraOffset = new THREE.Vector3(0, 2, 3)
    const offsetRotated = cameraOffset.clone().applyAxisAngle(new THREE.Vector3(0, 1, 0), rotation.current.x)

    camera.position.copy(playerRef.current.position.clone().add(offsetRotated))
